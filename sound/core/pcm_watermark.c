...
static __s16 watermark_block_bits_g[MAX_WATERMARK_BITS_FOR_BLOCK]; // 存储水印比特的缓冲区
...
static DEFINE_MUTEX(watermark_offset_mutex);  // 保护 watermark_bit_offset_g 的互斥锁
...
void update_watermark(const char *watermark_str)
{
    ...
    current_watermark.crc32 = crc32(0,
        (unsigned char *)&current_watermark.magic,
        sizeof(current_watermark) - sizeof(current_watermark.sync_header) - sizeof(current_watermark.crc32));
}
...
int convert_watermark_to_bits()
{
    ...
    __append_bytes_to_bits((const __u8 *)&current_watermark.sync_header, sizeof(current_watermark.sync_header), watermark_block_bits_g, &bit_idx, MAX_WATERMARK_BITS_FOR_BLOCK);
    
    // 2. Magic number (1 byte = 8 bits)
    __append_bytes_to_bits(&current_watermark.magic, sizeof(current_watermark.magic), watermark_block_bits_g, &bit_idx, MAX_WATERMARK_BITS_FOR_BLOCK);
    
    // 3. timestamp_raw (包含 timestamp_offset 和 reserved) (4 bytes = 32 bits)
    //    由于是 packed struct，位域会按照定义顺序排列。直接转换整个 u32 即可。
    __append_bytes_to_bits((const __u8 *)&current_watermark.timestamp_raw, sizeof(current_watermark.timestamp_raw), watermark_block_bits_g, &bit_idx, MAX_WATERMARK_BITS_FOR_BLOCK);

    // 4. MAC address (6 bytes = 48 bits)
    __append_bytes_to_bits(current_watermark.mac, sizeof(current_watermark.mac), watermark_block_bits_g, &bit_idx, MAX_WATERMARK_BITS_FOR_BLOCK);

    // 5. Watermark content (32 bytes = 256 bits)
    __append_bytes_to_bits(current_watermark.watermark_content, sizeof(current_watermark.watermark_content), watermark_block_bits_g, &bit_idx, MAX_WATERMARK_BITS_FOR_BLOCK);

    // 6. CRC32 (4 bytes = 32 bits)
    __append_bytes_to_bits((const __u8 *)&current_watermark.crc32, sizeof(current_watermark.crc32), watermark_block_bits_g, &bit_idx, MAX_WATERMARK_BITS_FOR_BLOCK);
